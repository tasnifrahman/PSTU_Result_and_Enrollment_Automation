...
def payment_success(request, student_id):
    try:
        # Fetch student by ID and update payment status
        student = Student.objects.get(student_id=student_id)
        student.payment_status = 'Paid'
        student.save()

        # Retrieve transaction details from POST request
        transaction_id = request.POST.get('tran_id')
        amount = request.POST.get('amount')

        # Set timezone and get the current time in BST
        bangladesh_timezone = pytz.timezone('Asia/Dhaka')
        current_time_bst = timezone.now().astimezone(bangladesh_timezone)

        # Create a new transaction record for the student
        ...
        from_email = f'Your University Team <{settings.EMAIL_HOST_USER}>'
